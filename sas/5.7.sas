...
* 拟合第一个残差序列 逐步回归 AR(2);
/*proc autoreg data=a;*/
/*model x=t/ nalg=5 dwprob backstep method=ml noint;*/
...
/*proc gplot data=out;*/
/*plot x*t=2 lcl*t=3 LCL_arch*t=4 ucl*t=3 UCL_arch*t=4 /overlay;*/
...
data out;
set out;
lcl_residul=-1.96*sqrt(20936);
Ucl_residul=1.96*sqrt(20936);
Lcl_GARCH=-1.96*sqrt(cev);
Ucl_GARCH=1.96*sqrt(cev);
Lcl_P=P-1.96*sqrt(cev);
Ucl_P=P+1.96*sqrt(cev);
proc gplot data=out;
plot x*t=2  lcl*t=3 ucl*t=3 Lcl_P*t=4 Ucl_P*t=4/overlay;
