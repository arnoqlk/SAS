...
;
proc gplot data=my;
plot sheep*year=1 difx*year=2;
symbol1 i=join v=star c=red;
symbol2 i=join v=star c=red;
run;
proc arima data=my;
identify var=sheep nlag=24;
identify var=sheep(1) nlag=24;
run;
proc autoreg data=my;
model sheep=lagx/nlag=5 lagdep=lagx dwprob archtest;
output out=out p=xp;
run;
proc arima data=my;
identify var=sheep nlag=24 minic p=(0:5) q=(0:5); 
estimate p=(3)(1);
forecast lead=7 id=year out=forecast;
run;
proc gplot data=forecast;
plot sheep*year=2 FORECAST*year=3/overlay;
